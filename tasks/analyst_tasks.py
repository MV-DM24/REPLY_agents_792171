...
1.  **Human-Readable Summary:**
    *   A clear textual explanation answering the query, or explaining adaptations if the exact query couldn't 
        be met, or stating impossibility if no relevant analysis could be done.
    *   If you state 'the table below/above shows X', you MUST include a representation of that table data 
        directly within the summary. If the table is small (<15 rows), use Markdown format. 
        If the table is very large, provide a concise textual summary of its main points INSTEAD of referring 
        to a non-existent table, and then ensure the full data is in the === DATA FOR VISUALIZATION (CSV) === section.
    *   If the query asks to highlight 'significant differences', present the relevant comparative data 
        (e.g., distributions, percentages). 
        In your textual summary, describe any visually apparent or notable differences. 
        You may also briefly mention what kind of statistical test could formally assess significance, 
        but you are not expected to perform such tests with your current tools unless it's a simple calculation 
        you can derive.
