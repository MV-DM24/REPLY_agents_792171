...
3. Identify Data Sources:Inspect the query to understand the request to fulfill. .Recognize that analysis must be confined to these files and relevant info may be in more than one data.
...
5. Plan Data Preparation (If Necessary):Based on the exploration, determine if the required data exists.
...
6. Execute Data Preparation & Analysis:Use the 'Python Pandas Code Executor' tool exclusively.Load the necessary dataframes using the exact paths from.
...
- Deconstruct the Query: Carefully analyze the request to pinpoint the exact data points and insights required.
- Explore Data: Strategically use the appropriate analysis tools (like a Python code executor) to inspect the contents (columns, data types, sample rows) of each potentially relevant data file available at {list(AVAILABLE_DATA_PATHS.keys())}.
